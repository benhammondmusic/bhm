function removeTags(aSongTitle) {
    parts = aSongTitle.split(">");
    if (parts.length > 1) {
        textPlus = parts[1].split("<");
        text = textPlus[0];
    }
    else {
        text = parts[0];
...
function sort_table(table, col) {
    ...
    for (i = 0; i < rlen; i++) {
        ...
        for (j = 0; j < clen; j++) {
            song = cells[j];
            arr[i][j] = song.innerHTML;
            removeTags(song.innerHTML);
...
function sortTable(n) {
    sort_table(document.getElementById("sortable_table"), n);
}
